    ...
    if let Some(namespace) = namespace {
        tokens.extend(
                quote! {{
                    ...
                    {
                        #children_tokens
                        sauron_core::prelude::html::element_ns(Some(#namespace), #name, [#(#attributes),*], children, #self_closing)
                    }
                }}
            );
    } else {
        tokens.extend(quote! {{
            ...
            {
                #children_tokens
                sauron_core::prelude::html::element_ns(None, #name, [#(#attributes),*], children, #self_closing)
            }
        }});
    }
    tokens
            ...
            match attribute.node_type {
                ...
                NodeType::Attribute => {
                    ...
                    if name.starts_with("on_") {
                        ...
                        quote::quote! {
                            sauron_core::prelude::html::attributes::attr(#name, #value)
                        }
                    }
                }
                _ => {
        ...
        for child in children {
            match child.node_type {
                ...
                NodeType::Text => {
                    ...
                    tokens.extend(quote! {
                        #receiver.push(sauron_core::prelude::html::text(#s));
                    });
                }
                NodeType::Comment => {
                    let s = child
