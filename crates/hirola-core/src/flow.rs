...
pub struct Keyed<T: 'static, F: 'static + Clone, G, K: 'static + Clone, Key: 'static>
where
    F: Fn(T) -> TemplateResult<G>,
    K: Fn(&T) -> Key,
    Key: Hash + Eq,
    G: GenericNode,
{
    pub props: KeyedProps<T, F, G, K, Key>,
}
...
impl<
        ...
        K: 'static + Clone,
        Key: 'static + Clone,
    > Render<G> for Keyed<T, F, G, K, Key>
where
    F: Fn(T) -> TemplateResult<G>,
    K: Fn(&T) -> Key,
    Key: Hash + Eq,
            ...
            move || {
                ...
                for (i, item) in iterable.get().iter().enumerate() {
                    ...
                    } != item
                    {
                        ...
                        let old_owner =
                            mem::replace(old_owner, Owner::new() /* placeholder */);
                        drop(old_owner);
...
///
#[derive(Debug)]
pub struct IndexedProps<T: 'static + Clone, F, G: GenericNode>
where
    F: Fn(T) -> TemplateResult<G>,
{
    pub iterable: StateHandle<Vec<T>>,
...
pub struct Indexed<T: 'static + Clone, F, G: GenericNode>
where
    F: Fn(T) -> TemplateResult<G>,
{
    pub props: IndexedProps<T, F, G>,
}

impl<T, F, G> Render<G> for Indexed<T, F, G>
where
    F: Fn(T) -> TemplateResult<G> + 'static + Clone,
    G: GenericNode,
    T: Clone + PartialEq + 'static,
{
    fn render(&self) -> TemplateResult<G> {
        ...
        let iterable = props.iterable.clone();
        ...
        create_effect({
            ...
            move || {
                ...
                for (i, item) in iterable.get().iter().enumerate() {
                    ...
                    if previous_value.is_none() || previous_value.unwrap() != item {
                        ...
                        if templates.borrow().get(i).is_some() {
                            ...
                            debug_assert!(
                                templates.borrow().len() == i,
                                "pushing new value scenario"
                            );
                ...
                if templates.borrow().len() > iterable.get().len() {
                    let mut templates = templates.borrow_mut();
                    let excess_nodes = templates.drain(iterable.get().len()..);
                ...
                *previous_values.borrow_mut() = (*iterable.get()).clone();
            }
        });
